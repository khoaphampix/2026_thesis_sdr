    ...
    try:
        ...
        while base <= total_packets:
            ...
            for attempt in range(1, args.retries + 1):
                ...
                burst_rf_seconds = len(iq) / args.sample_rate
                burst_slot = max(
                    args.data_slot,
                    burst_rf_seconds * args.burst_repeat_factor,
                )
                logger.info(
                    "BURST TIMING packets=%d rf=%.3fms hold=%.3fms",
                    len(missing_sequences),
                    burst_rf_seconds * 1000,
                    burst_slot * 1000,
                )
                send_cyclic_burst(device, iq, burst_slot)
                total_bursts += 1
                time.sleep(args.turnaround_guard)
