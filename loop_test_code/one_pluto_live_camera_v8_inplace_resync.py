#!/usr/bin/env python3
"""
one_pluto_live_camera_v8_inplace_resync.py
...
Exactly one adi.Pluto context remains open for the whole run. Recovery is performed in-place by rebuilding buffers and reapplying the radio configuration.
"""
...
def build_packet(
    ...
    # Fixed packet size makes SDR recovery simpler.
    return header + payload.ljust(payload_size, b"\x00")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "One-Pluto H.265 transmission with robust synchronization and in-place long-run radio recovery."
        )
    )
                        ...
                        try:
                            ...
                            for capture_number in range(1, args.captures_per_attempt + 1):
                                ...
                                try:
                                    ...
                                    received_payload = fields["payload"]
                                    success = True
                                    break
